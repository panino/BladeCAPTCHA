async function initCaptcha(config) {
    ...
    function callOnce(kind, ...args) {
        if (kind === 'onSuccess') {
            if (callbacksFired.success) return;
            callbacksFired.success = true;
            if (typeof config.onSuccess === 'function') {
                try { config.onSuccess(...args); } catch (e) { console.error('onSuccess error', e); }
            }
        } else if (kind === 'onError') {
            if (callbacksFired.error) return;
            callbacksFired.error = true;
            if (typeof config.onError === 'function') {
                try { config.onError(...args); } catch (e) { console.error('onError error', e); }
            }
        } else if (kind === 'onEnd') {
            if (callbacksFired.cancel) return;
            callbacksFired.cancel = true;
            if (typeof config.onEnd === 'function') {
                try { config.onEnd(...args); } catch (e) { console.error('onEnd error', e); }
            }
        } else if (kind === 'onStart') {
            if (callbacksFired.load) return;
            callbacksFired.load = true;
            if (typeof config.onStart === 'function') {
                try { config.onStart(...args); } catch (e) { console.error('onStart error', e); }
            }
        }
    }
    ...
    async function handleVerification() {
        ...
        enProceso = true;
        callbacksFired = { success: false, error: false, cancel: false, load: false };
        if (verifyButton) {
